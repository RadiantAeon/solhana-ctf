...
pub mod challenge1 {
    ...
    /// transfers tokens from depositor token account to program owned(?) token vault
    /// mints voucher tokens to user
...
pub struct Transact<'info> {
    ...
    pub deposit_account: Account<'info, TokenAccount>,
    // solve: make new mint -> mint a bunch of tokens for us -> transfer authority to program
